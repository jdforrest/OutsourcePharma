...
function cc_widgets_init() {
    ...
    register_sidebars(1, array(
        'name' => 'Front Page Bottom Center',
        'id' => 'frontpagebottomcenter',
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget' => '</div><div class="clear"></div>',
        'before_title' => '<h3 class="widgettitle">',
        'after_title' => '</h3>'
...
//extra code to default the Groups view to grid without affecting other object types - MF
...
function cc_get_displaymode($object) {
    ...
    if (isset($_BP_COOKIE['bp-' . $object . '-displaymode'])) {
        get_template_part("{$object}/{$object}-loop", $_BP_COOKIE['bp-' . $object . '-displaymode']);
			...
			if ($object='groups') {
				get_template_part("{$object}/{$object}-loop", 'grid');
			} else {
...
/* Suppress first two levels of breadcrumbs. */
...
function my_login_logo() { ?>
    <style type="text/css">
        body.login div#login h1 a {
            background-image: url(<?php echo get_stylesheet_directory_uri(); ?>/images/life-science-strategy-mark-transparent.png);
...
/* Make the Name validation error more user-friendly. */

function ChangeNameValidationError() {
	
	global $bp;
	$bph_field_txt = $_POST['field_1'];
	
	if (empty($bph_field_txt) || $bph_field_txt == '') {
		$bp->signup->errors['field_1'] = __('Please enter your name.','buddypress');
	}
	return;
}
